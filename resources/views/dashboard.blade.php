        ...
        <div class="row justify-content-center">
            ...
            <div class="container-fluid px-4">
                ...
                <ol class="breadcrumb mb-4">
                    {{-- chtgpt --}}
                    @php
                        $trialDate = \Carbon\Carbon::parse(auth()->user()->user_trial);
                    @endphp

                    Hello, {{ auth()->user()->name }}
                    @if (!auth()->user()->billing_ends)
                        <p class="text-danger">Please update your billing address to continue using the service.</p>

                        @if (Auth::check() && auth()->user()->user_type == 'employer')
                            <p>Your trial {{ now()->greaterThan($trialDate) ? 'was expired' : 'will expire' }} on
                                {{ $trialDate->format('Y-m-d') }}</p>
                        @endif
                    @endif
                    @if (Auth::check() && auth()->user()->user_type == 'employer')
                        <p>Your trial {{ now()->greaterThan($trialDate) ? 'was expired' : 'will expire' }} on
